...
#warning [NSScreen mainScreen].frame
        // init video sink
        _glView = [[IJKSDLGLView alloc] initWithFrame:NSMakeRect(0, 0, 1024, 576)];
...
- (void)setScalingMode: (IJKMPMovieScalingMode) aScalingMode
{
    ...
    switch (aScalingMode) {
        case IJKMPMovieScalingModeNone:
            [_view setLayerContentsPlacement:NSViewLayerContentsPlacementCenter];
            //[_view setContentMode:UIViewContentModeCenter];
            break;
        case IJKMPMovieScalingModeAspectFit:
            [_view setLayerContentsPlacement:NSViewLayerContentsPlacementScaleProportionallyToFit];
            //[_view setContentMode:UIViewContentModeScaleAspectFit];
            break;
        case IJKMPMovieScalingModeAspectFill:
        case IJKMPMovieScalingModeFill:
            [_view setLayerContentsPlacement:NSViewLayerContentsPlacementScaleProportionallyToFill];
            break;
        default:
...
- (NSImage *)thumbnailImageAtTime:(NSTimeInterval)playbackTime timeOption:(IJKMPMovieTimeOption)option
{
    return nil;
}
...
- (void)setZzPlaybackRate:(float)zzPlaybackRate
{
    if (!_mediaPlayer)
        return;
    
    return ijkmp_set_zzplayback_rate(_mediaPlayer, zzPlaybackRate);
}

- (float)zzPlaybackRate
{
    if (!_mediaPlayer)
        return 0.0f;
    
    return ijkmp_get_zzplayback_rate(_mediaPlayer);
}
...
- (BOOL)realtime
{
    ...
    return ijkmp_is_realtime(_mediaPlayer) == 1 ? YES : NO;
}
...
{
    //NSLog(@"IJKFFMoviePlayerController:applicationDidBecomeActive: %d", (int)[NSApplication sharedApplication].applicationState);
}

- (void)applicationWillResignActive
{
    //NSLog(@"IJKFFMoviePlayerController:applicationWillResignActive: %d", (int)[NSApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
...
{
    //NSLog(@"IJKFFMoviePlayerController:applicationDidEnterBackground: %d", (int)[NSApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_pauseInBackground) {
            [self pause];
        }
...
{
    //NSLog(@"IJKFFMoviePlayerController:applicationWillTerminate: %d", (int)[NSApplication sharedApplication].applicationState);
    dispatch_async(dispatch_get_main_queue(), ^{
        if (_pauseInBackground) {
            [self pause];
        }
