...
//    printf("CreateSampleBufferFrom %zu\n",demux_size);
//    for(int i = 0; i < demux_size;++i){
//        printf("0x%02x ",((uint8 *)demux_buff)[i]);
//    }
//    printf("\n");
    uint8 *buf = (uint8 *)demux_buff;
    int32_t origin_len = buf[0] << 24 | buf[1] << 16 | buf[2] << 8 | buf[3];
//    printf("CreateSampleBufferFrom %zu %d\n",demux_size,origin_len);
//    //if(origin_len != demux_size - 4){
//        printf("CCCCCCCCCCreateSampleBufferFrom %zu\n",demux_size);
//        for(int i = 0; i < 60;++i){
//            printf("0x%02x ",((uint8 *)demux_buff)[i]);
//        }
//        printf("\n");
...
//static const uint8_t *zz_avc_find_startcode_internal(const uint8_t *p, const uint8_t *end)
//{
...
//}
//
//const uint8_t *zz_avc_find_startcode(const uint8_t *p, const uint8_t *end){
//    const uint8_t *out= zz_avc_find_startcode_internal(p, end);
//    if(p<out && out<end && !out[-1]) out--;
//    return out;
//}
//
//int zz_avc_parse_nal_units(AVIOContext *pb, const uint8_t *buf_in, int size)
//{
...
//    
//    size = 0;
//    nal_start = zz_avc_find_startcode(p, end);
//    for (;;) {
//        while (nal_start < end && !*(nal_start++));
//        if (nal_start == end)
//            break;
//        
//        nal_end = zz_avc_find_startcode(nal_start, end);
...
static int decode_video_internal(Ijk_VideoToolBox_Opaque* context, AVCodecContext *avctx, const AVPacket *avpkt, int* got_picture_ptr)
{
    ...
    if (context->fmt_desc.convert_bytestream) {
        ...
        //ff_avc_parse_nal_units(pb, pData_temp, iSize_temp);
