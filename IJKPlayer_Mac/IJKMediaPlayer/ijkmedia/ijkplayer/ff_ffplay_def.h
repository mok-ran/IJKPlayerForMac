...
typedef struct FFPlayer {
    ...
    float zz_playback_rate;
    int   zz_playback_rate_changed;
    
    
} FFPlayer;

#define ZZ_PLAYBACK_RATE_MAX     5.0f
#define ZZ_PLAYBACK_RATE_MIN     0.2f
#define ZZ_PLAYBACK_RATE_DEFAULT 1.0f

inline static int ffp_set_zz_audio_sample(FFPlayer *ffp,int value){

    ffp->zz_playback_rate = av_clipf_c(ffp->zz_playback_rate, ZZ_PLAYBACK_RATE_MIN, ZZ_PLAYBACK_RATE_MAX);
    
    int r_value = value;
    if (fabsf(ffp->zz_playback_rate - ZZ_PLAYBACK_RATE_DEFAULT) > 0.0001){
        r_value = value / ffp->zz_playback_rate;
    }
    
    return r_value;
}
inline static float ffp_set_zz_video_duration(FFPlayer *ffp,float value){
...
inline static void ffp_reset_internal(FFPlayer *ffp)
{
    ...
    ffp->zz_playback_rate = ZZ_PLAYBACK_RATE_DEFAULT;
    ffp->zz_playback_rate_changed = 0;
