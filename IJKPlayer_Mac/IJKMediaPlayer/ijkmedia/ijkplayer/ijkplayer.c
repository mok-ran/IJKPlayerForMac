...
void ijkmp_set_zzplayback_rate(IjkMediaPlayer *mp, float rate)
{
    assert(mp);
    
    MPTRACE("%s(%f)\n", __func__, rate);
    pthread_mutex_lock(&mp->mutex);
    ffp_set_zzplayback_rate(mp->ffplayer, rate);
    pthread_mutex_unlock(&mp->mutex);
    MPTRACE("%s()=void\n", __func__);
}

float ijkmp_get_zzplayback_rate(IjkMediaPlayer *mp)
{
    assert(mp);
    float value = 0;
    pthread_mutex_lock(&mp->mutex);
    value = mp->ffplayer->zz_playback_rate;
    pthread_mutex_unlock(&mp->mutex);
    return value;
}

int ijkmp_is_realtime(IjkMediaPlayer *mp)
{
    assert(mp);
    int value = 0;
    pthread_mutex_lock(&mp->mutex);
    value = mp->ffplayer->is->realtime;
    pthread_mutex_unlock(&mp->mutex);
    return value;
}
