...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            last_duration = ffp_set_zz_video_duration(ffp, last_duration);
            ...
            if (frame_queue_nb_remaining(&is->pictq) > 1) {
                ...
                duration = ffp_set_zz_video_duration(ffp, duration);
...
reload:
    ...
    wanted_nb_samples = ffp_set_zz_audio_sample(ffp, wanted_nb_samples);
...
void ffp_set_zzplayback_rate(FFPlayer *ffp, float rate)
{
    if (!ffp)
        return;
    
    av_log(ffp, AV_LOG_INFO, "ZZPlayback rate: %f\n", rate);
    ffp->zz_playback_rate = rate;
    ffp->zz_playback_rate_changed = 1;
}
